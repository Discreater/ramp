...
pub struct MtgyModulus<'a> {
    modulus: &'a Int,
    modulus_inv0: crate::ll::limb::Limb,
    limbs: usize,
    r: Int,
}
...
impl<'a> MtgyModulus<'a> {
    ...
    fn redc(&self, a: &mut Int) {
        unsafe {
            ...
            crate::ll::copy_incr(a.limbs(), t.limbs_uninit(), a.abs_size());
            for i in a.abs_size()..(t.cap as i32) {
                *t.limbs_uninit().offset(i as isize) = crate::ll::limb::Limb(0);
            }
            crate::ll::mtgy::redc(
                a.limbs_uninit(),
                self.limbs as i32,
    ...
    pub fn sqr(&self, a: &MtgyInt) -> MtgyInt {
        unsafe {
            ...
            let mut t = Int::with_capacity(2 * self.limbs as u32);
            t.size = t.cap as i32;
            crate::ll::sqr(t.limbs_uninit(), a.0.limbs(), self.limbs as i32);
            self.redc(&mut t);
            MtgyInt(t)
        }
    }
    ...
    pub fn pow(&self, basis: &MtgyInt, exponent: &Int) -> MtgyInt {
        ...
        unsafe {
            ...
            crate::ll::mtgy::modpow(
                result.0.limbs_uninit(),
                self.limbs as i32,
                self.modulus.limbs(),
                self.modulus_inv0,
                basis.0.limbs(),
    ...
    fn pad_to(a: &mut Int, s: usize) {
        unsafe {
            ...
            for i in a.abs_size()..(a.cap as i32) {
                *a.limbs_uninit().offset(i as isize) = crate::ll::limb::Limb(0);
            }
            a.size = s as i32;
        }
    }
    ...
    pub fn to_int(&self, a: &MtgyInt) -> Int {
        ...
        let mut it = unsafe {
            ...
            crate::ll::copy_incr(a.0.limbs(), it.limbs_uninit(), self.limbs as i32);
...
fn cvt() {
    let cases = [
        ...
        ("15", "1009"),
        (
            "9330786055998253486590",
            "4349330786055998253486590232462401",
        ),
        ("7", "9"),
